...
void Detector::ProcessLoop() {
	...
	std::cout
			<< "Esperando Pela finalização da thread de processamento na main\n";
	...
	while (running) {
		if (sensor.ReadSensor() != 0) {
...

			{
				...
				// Isso garante que a imagem apareça na tela mesmo se a impressora
				// falhar.
...
cv::Mat Detector::GetFrame() {
	std::lock_guard<std::mutex> lock(frame_mutex);
	if (frame.empty())
		return cv::Mat(); // Retorna vazio se não houver frame
	return frame.clone();
}
