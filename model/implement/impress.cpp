...
void Impress::LoadAtributes()
{
    ...
    if (arq.dict.count("espacamento"))
        strcpy(espacamento, arq.dict["espacamento"].c_str());
    if (arq.dict.count("densidade"))
        densidade = std::stoi(arq.dict["densidade"]);
    if (arq.dict.count("velocidade"))
        velocidade = std::stoi(arq.dict["velocidade"]);
    if (arq.dict.count("direcao"))
        direcao = std::stoi(arq.dict["direcao"]);
    if (arq.dict.count("texto"))
        strcpy(texto, arq.dict["texto"].c_str());
    if (arq.dict.count("tamanho_fonte"))
        strcpy(tamanho_fonte, arq.dict["tamanho_fonte"].c_str());
    if (arq.dict.count("posicao_x"))
        posicao_x = std::stoi(arq.dict["posicao_x"]);
    if (arq.dict.count("posicao_y_lote"))
        posicao_y_lote = std::stoi(arq.dict["posicao_y_lote"]);
    if (arq.dict.count("posicao_y_fabricacao"))
        posicao_y_fabricacao = std::stoi(arq.dict["posicao_y_fabricacao"]);
    if (arq.dict.count("posicao_y_validade"))
        posicao_y_validade = std::stoi(arq.dict["posicao_y_validade"]);
